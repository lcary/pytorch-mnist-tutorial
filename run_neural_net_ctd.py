...
def main() -> None:
    ...
    loss_data_file = os.path.join(args.out_dir, 'loss_data.json')

    with open(loss_data_file, 'r') as infile:
        data = json.load(infile)

    train_losses = data['train_losses']
    train_counter = data['train_counter']
    test_losses = data['test_losses']
    test_counter = data['test_counter']
    test_counter.extend([i * len(train_loader.dataset) for i in range((end_range - start_range) + 1)])
